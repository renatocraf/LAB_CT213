...
class NeuralNetwork:
    ...
    def forward_propagation(self, inputs):
        ...
        for i in range(len(z)-1):
            ...
            a[i+1] = sigmoid(z[i+1])
        #a[2] = 0.001 * np.ones((self.num_outputs, inputs.shape[1]))  # Change this line k,
    ...
    def back_propagation(self, inputs, expected_outputs):
        ...
        for i in range(len(self.weights)-1):
            self.weights[i+1] = self.weights[i+1] - self.alpha * weights_gradient[i+1]
            self.biases[i+1]  = self.biases[i+1] - self.alpha * biases_gradient[i+1]
