...
#camada1
model.add(layers.Dense(num_neurons1, activation=activations.linear, input_shape=(1,), kernel_regularizer=regularizers.l2(lambda_l2)))
model.add(layers.LeakyReLU(alpha)) # alpha is the Leaky ReLU parameter
#camada2
model.add(layers.Dense(num_neurons2, activation=activations.linear, kernel_regularizer=regularizers.l2(lambda_l2)))
model.add(layers.LeakyReLU(alpha)) # alpha is the Leaky ReLU parameter
#camada3
model.add(layers.Dense(num_neurons3, activation=activations.linear, kernel_regularizer=regularizers.l2(lambda_l2)))
